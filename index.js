...
async function showMainMenu(ctx) {
  ...
  await sendOrEdit(ctx, greeting, user.is_admin ? mainMenuAdmin() : mainMenu());
}
...
async function notifyAdminsDeposit(ctx, depositRow) {
  ...
  for (const a of admins) {
    try { await ctx.telegram.sendPhoto(a.id, depositRow.screenshot_file_id, { caption: text, ...kb }); } catch { /* ignore */ }
  }
}
...
async function showCategory(ctx, parentId, page, backTo) {
  ...
  if (!visibleCats.length && !visibleProds.length && !vcBtns.length && !manualBtns.length) {
    ...
    else emptyRows.push([Markup.button.callback(backLabel, backTo === 0 ? "home" : `cat:${backTo}:1:0`), Markup.button.callback(homeLabel, "home")]);
  ...
  else rows.push([Markup.button.callback(backLabel, backTo === 0 ? "home" : `cat:${backTo}:1:0`), Markup.button.callback(homeLabel, "home")]);
