...
function repairArabicEncoding(value) {
  ...
  const replacements = {
    "طھظ…": "تم", "ط±ظپط¶": "رفض", "ط£ط­ط¯": "أحد",
    "ط·ظ„ط¨ط§طھظƒ": "طلباتك", "ط§ظ„ظ…ظ†طھط¬": "المنتج",
    "ط§ظ„ط±ط¯": "الرد", "طھظ…طھ": "تمت", "ط¥ط¹ط§ط¯ط©": "إعادة",
    "ط±طµظٹط¯ظƒ": "رصيدك", "ط§ظ„ظ…ط³طھط®ط¯ظ…": "المستخدم",
    "â‌Œ": "❌", "ًں›’": "🛒", "ًں’°": "💰", "ًں“‹": "📋",
    "ًں”‘": "🔑", "ًں“¦": "📦", "âœ…": "✅",
  };
...
async function ensurePrimaryApiSource() {
  ...
  const inserted = await q(
    ...
    ["API الأساسي", normalizeApiBase(ORANOS_BASE), encryptApiToken(ORANOS_TOKEN)]
  );
...
async function syncApiSource(source) {
  ...
  const productsResponse = await client.get("/client/api/products");
  ...
  for (const raw of products) {
    ...
    await q(
      ...
      [
        botId, source.id, externalId, repairArabicEncoding(String(raw.name ?? `منتج ${externalId}`)), parentId,
...
async function placeOrderForProduct(product, params, orderUuid) {
  ...
  if (!source || !source.active) return { status: "ERR", message: "مصدر المنتج غير متاح" };
...
//  PRODUCT CACHE
// ============================================================
const PRODUCTS_TTL  = 2 * 60_000;    // الكاش سريع مع بقاء الاستجابة فورية
...
function startBackgroundRefresher() {
  ...
  setInterval(() => {
    // مزامنة قاعدة المنتجات بالخلفية، وتبقى الاستجابة من الكاش/قاعدة البيانات فورية
    Promise.all([
      syncAllApiSources(),
...
async function showDepositMethod(ctx, methodId) {
  ...
  const infoText = `💳 ${m.name}\n🔑 الرقم: \`${m.identifier}\`\n\n📋 التعليمات:\n${m.instructions}\n\n📤 أرسل المبلغ وصورة إشعار التحويل (يمكنك إرسالهما بأي ترتيب).`;
  if (m.image_file_id) {
...
async function executeOrder(ctx) {
  ...
  if (orderExecutionLocks.has(uid)) {
    await ctx.answerCbQuery("⏳ طلبك قيد التنفيذ...").catch(() => {});
    return;
  }
...
async function executeOrderInternal(ctx) {
  ...
  if (!debited) {
    setStep(ctx.from.id, { kind: "idle" });
    await ctx.reply("❌ تغيّر الرصيد قبل تنفيذ الطلب. حدّث الرصيد وحاول مجدداً.", Markup.inlineKeyboard([[Markup.button.callback("🏠 الرئيسية", "home")]]));
    return;
  }
...
async function showAdminMenu(ctx) {
  ...
  const rows = [
    ...
    [Markup.button.callback("🔌 APIs المنتجات", "adm:apis"), Markup.button.callback("🔄 بينج تلقائي", "adm:ping")],
    [Markup.button.callback(status === "on" ? "🟢 البوت: شغال" : "🔴 البوت: متوقف", "adm:toggleStatus")],
...
async function showApiSources(ctx) {
  ...
  const rows = sources.map(s => [
    Markup.button.callback(`${s.active ? "🟢" : "🔴"} ${s.name}${s.is_primary ? " ⭐" : ""}`.slice(0, 60), `adm:api:${s.id}`)
  ]);
  rows.push([Markup.button.callback("➕ إضافة API منتجات", "adm:apiAdd")]);
  rows.push([Markup.button.callback("🔄 مزامنة كل المنتجات الآن", "adm:apiSyncAll")]);
  rows.push([Markup.button.callback("⬅️ رجوع", "admin:menu")]);
  await sendOrEdit(ctx, "🔌 APIs المنتجات\n\nيتم حفظ المنتجات في قاعدة البيانات ومزامنتها تلقائياً.", Markup.inlineKeyboard(rows));
}
...
async function showUserCard(ctx, uid) {
  ...
  const kb = [
    ...
    [Markup.button.callback("💬 مراسلة المستخدم خاص", `adm:userMsg:${uid}`)],
  ];
...
async function startBot() {
  ...
  bot.action(/^adm:userMsg:(\d+)$/, async ctx => {
    if (!(await requireAdmin(ctx))) return;
    const uid = Number(ctx.match[1]); const u = await getUser(uid);
    if (!u) { await ctx.reply("⚠️ المستخدم غير موجود."); return; }
    setStep(ctx.from.id, { kind: "admin:userMessage", userId: uid });
    await ctx.reply(`💬 أرسل الرسالة الخاصة إلى ${u.first_name ?? uid}:\nستصل إليه من البوت مباشرة.`, Markup.inlineKeyboard([[Markup.button.callback("❌ إلغاء", `adm:user:${uid}`)]]));
  });
  ...
  bot.action("adm:apiAdd", async ctx => {
    ...
    await ctx.reply("🔌 أرسل اسم الـAPI الجديد:", Markup.inlineKeyboard([[Markup.button.callback("❌ إلغاء", "adm:apis")]]));
  });
  bot.action(/^adm:api:(\d+)$/, async ctx => {
    if (!(await requireAdmin(ctx))) return;
    const source = await getApiSource(Number(ctx.match[1]));
    if (!source) { await ctx.reply("⚠️ مصدر الـAPI غير موجود."); return; }
    const lastSync = source.last_sync_at ? new Date(source.last_sync_at).toLocaleString("ar") : "لم تتم المزامنة";
    const error = source.last_sync_error ? `\n⚠️ آخر خطأ: ${source.last_sync_error}` : "";
    const buttons = [];
    if (!source.is_primary) {
      buttons.push([Markup.button.callback(source.active ? "🔴 تعطيل" : "🟢 تفعيل", `adm:apiToggle:${source.id}`), Markup.button.callback("🗑️ حذف", `adm:apiDel:${source.id}`)]);
    }
    buttons.push([Markup.button.callback("🔄 مزامنة الآن", `adm:apiSync:${source.id}`)]);
    buttons.push([Markup.button.callback("⬅️ رجوع", "adm:apis")]);
    await sendOrEdit(ctx,
      `🔌 ${source.name}${source.is_primary ? " ⭐ (الأساسي)" : ""}\nالرابط: ${source.base_url}\nالحالة: ${source.active ? "مفعّل" : "موقوف"}\nآخر مزامنة: ${lastSync}${error}`,
      Markup.inlineKeyboard(buttons));
  });
  bot.action(/^adm:apiToggle:(\d+)$/, async ctx => {
    if (!(await requireAdmin(ctx))) return;
    const id = Number(ctx.match[1]); const source = await getApiSource(id);
    if (!source || source.is_primary) { await ctx.reply("⚠️ لا يمكن تعطيل الـAPI الأساسي."); return; }
  ...
  bot.action(/^adm:apiDel:(\d+)$/, async ctx => {
    if (!(await requireAdmin(ctx))) return;
    const id = Number(ctx.match[1]); const source = await getApiSource(id);
    if (!source || source.is_primary) { await ctx.reply("⚠️ لا يمكن حذف الـAPI الأساسي."); return; }
    await q("DELETE FROM cached_products WHERE source_id=$1", [id]);
    await q("DELETE FROM api_sources WHERE id=$1", [id]);
    invalidateCaches(); await ctx.reply("✅ تم حذف API ومنتجاته من الكتالوج."); await showApiSources(ctx);
  });
  ...
  bot.on("photo", async ctx => {
    ...
    if (step.kind === "deposit:info") {
      // إذا أرسل المستخدم المبلغ في تعليق الصورة، نقرأه مباشرة.
  ...
  bot.on("text", async (ctx, next) => {
    ...
    if (step.kind === "order:manualNote") {
      ...
      if (!m) return;
       ...
       if (!debited) {
         setStep(ctx.from.id, { kind: "idle" });
         await ctx.reply("❌ رصيد غير كافٍ حالياً. حاول تحديث الرصيد ثم أعد الطلب.");
         return;
       }
    ...
    switch (step.kind) {
      ...
      case "admin:userMessage": {
        if (!txt) { await ctx.reply("⚠️ الرسالة فارغة."); return; }
        try {
          await ctx.telegram.sendMessage(step.userId, `📩 رسالة من الإدارة:\n\n${txt}`);
          await q("INSERT INTO admin_messages(admin_id,user_id,message) VALUES($1,$2,$3)", [ctx.from.id, step.userId, txt]);
          setStep(ctx.from.id, { kind: "idle" });
          await ctx.reply("✅ تم إرسال الرسالة للمستخدم بشكل خاص.");
        } catch {
          setStep(ctx.from.id, { kind: "idle" });
          await ctx.reply("❌ تعذر إرسال الرسالة. ربما المستخدم حظر البوت.");
        }
        return;
      }
      ...
      case "admin:addApi:name": {
        setStep(ctx.from.id, { kind: "admin:addApi:url", name: txt });
        await ctx.reply("🌐 أرسل رابط API الأساسي بدون / في النهاية\nمثال: https://example.com:", Markup.inlineKeyboard([[Markup.button.callback("❌ إلغاء", "adm:apis")]]));
        return;
      }
      case "admin:addApi:url": {
        const baseUrl = normalizeApiBase(txt);
        if (!/^https?:\/\/[^\s]+$/i.test(baseUrl)) {
          await ctx.reply("⚠️ الرابط غير صالح. أرسله بصيغة https://example.com");
          return;
        }
        setStep(ctx.from.id, { kind: "admin:addApi:token", name: step.name, baseUrl });
        await ctx.reply("🔑 أرسل API token. لن يظهر للمستخدمين وسيُحفظ مشفراً:", Markup.inlineKeyboard([[Markup.button.callback("❌ إلغاء", "adm:apis")]]));
        return;
      }
      case "admin:addApi:token": {
        if (!txt) { await ctx.reply("⚠️ الـAPI token لا يمكن أن يكون فارغاً."); return; }
        const inserted = await q(
          "INSERT INTO api_sources(name,base_url,token_encrypted,active,is_primary) VALUES($1,$2,$3,true,false) RETURNING id",
          [step.name, step.baseUrl, encryptApiToken(txt)]
        );
        setStep(ctx.from.id, { kind: "idle" });
        try {
          ...
          const count = await syncApiSource(source);
          invalidateCaches();
          await ctx.reply(`✅ تمت إضافة ${step.name} ومزامنة ${count} منتج.`);
        } catch {
          await ctx.reply("✅ تمت إضافة الـAPI، لكن فشلت أول مزامنة. يمكنك إعادة المحاولة من لوحة APIs.");
        }
        await showApiSources(ctx);
        return;
      }
  ...
  bot.catch(async (err, ctx) => {
    ...
    try {
      if (ctx?.callbackQuery) await ctx.answerCbQuery("⚠️ حدث خطأ مؤقت، حاول مرة أخرى.").catch(() => {});
      else if (ctx?.chat) await ctx.reply("⚠️ حدث خطأ مؤقت. حاول مرة أخرى بعد لحظات.").catch(() => {});
