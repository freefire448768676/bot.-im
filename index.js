...
function apiProductForDb(row) {
  ...
  return {
    ...
    category_name: row.category_name || raw.category_name || "",
    name: row.name,
...
async function syncApiSource(source) {
  ...
  for (const raw of products) {
    ...
    await q(
      ...
      [
        botId, source.id, externalId, String(raw.name ?? `ظ…ظ†طھط¬ ${externalId}`), parentId,
        raw.category_name ?? raw.categoryName ?? null, rawPrice,
...
async function getCachedContent(parentId) {
  ...
  const p = (async () => {
    ...
    if (extraSources.length) {
      content.categories = [
        ...(content.categories || []),
        ...extraSources.map(s => ({ id: sourceRootCategoryId(s.id), name: `ًں“، ${s.name}`, parent_id: 0 })),
      ];
    }
    return content;
  })()
...
async function startBot() {
  ...
  // ظ„ط§ ظ†ظˆظ‚ظپ طھط´ط؛ظٹظ„ ط§ظ„ط¨ظˆطھ ط¥ط°ط§ ظƒط§ظ† ط£ط­ط¯ ط§ظ„ظ€APIs ظ…طھظˆظ‚ظپط§ظ‹ط› ط§ظ„ظƒطھط§ظ„ظˆط¬ ط§ظ„ظ‚ط¯ظٹظ… ظٹط¨ظ‚ظ‰ ظ…طھط§ط­ط§ظ‹
  ...
  bot.on("text", async (ctx, next) => {
    ...
    switch (step.kind) {
      ...
      case "admin:addApi:name": {
        ...
        await ctx.reply("ًںŒگ ط£ط±ط³ظ„ ط±ط§ط¨ط· API ط§ظ„ط£ط³ط§ط³ظٹ ط¨ط¯ظˆظ† / ظپظٹ ط§ظ„ظ†ظ‡ط§ظٹط©\nظ…ط«ط§ظ„: https://example.com:", Markup.inlineKeyboard([[Markup.button.callback("â‌Œ ط¥ظ„ط؛ط§ط،", "adm:apis")]]));
        return;
      }
      case "admin:addApi:url": {
        const baseUrl = normalizeApiBase(txt);
        if (!/^https?:\/\/[^\s]+$/i.test(baseUrl)) {
          await ctx.reply("âڑ ï¸ڈ ط§ظ„ط±ط§ط¨ط· ط؛ظٹط± طµط§ظ„ط­. ط£ط±ط³ظ„ظ‡ ط¨طµظٹط؛ط© https://example.com");
          return;
        }
        setStep(ctx.from.id, { kind: "admin:addApi:token", name: step.name, baseUrl });
        await ctx.reply("ًں”‘ ط£ط±ط³ظ„ API token. ظ„ظ† ظٹط¸ظ‡ط± ظ„ظ„ظ…ط³طھط®ط¯ظ…ظٹظ† ظˆط³ظٹظڈط­ظپط¸ ظ…ط´ظپط±ط§ظ‹:", Markup.inlineKeyboard([[Markup.button.callback("â‌Œ ط¥ظ„ط؛ط§ط،", "adm:apis")]]));
        return;
      }
      case "admin:addApi:token": {
        ...
        try {
          ...
          await ctx.reply("âœ… طھظ…طھ ط¥ط¶ط§ظپط© ط§ظ„ظ€APIطŒ ظ„ظƒظ† ظپط´ظ„طھ ط£ظˆظ„ ظ…ط²ط§ظ…ظ†ط©. ظٹظ…ظƒظ†ظƒ ط¥ط¹ط§ط¯ط© ط§ظ„ظ…ط­ط§ظˆظ„ط© ظ…ظ† ظ„ظˆط­ط© APIs.");
        }
        await showApiSources(ctx);
        return;
      }
